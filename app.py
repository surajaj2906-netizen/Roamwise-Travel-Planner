...
class Itinerary(db.Model):
    ...
    passenger_details = db.Column(db.Text, nullable=True)
...
def send_transaction_email(recipient, subject, body, attachment=None):
...
def book_package(package_index):
    if not request.form.get("passenger_name", "").strip():
        return redirect(url_for("passenger_details", package_index=package_index))
    passenger_details = {
        "name": request.form.get("passenger_name", "").strip(),
        "age": request.form.get("passenger_age", "").strip(),
        "gender": request.form.get("passenger_gender", "").strip(),
        "phone": request.form.get("passenger_phone", "").strip(),
        "email": request.form.get("passenger_email", "").strip().lower(),
        "id_type": request.form.get("id_type", "").strip(),
        "id_last4": request.form.get("id_last4", "").strip(),
        "payment_mode": request.form.get("payment_mode", "").strip(),
    }
    try:
        age = int(passenger_details["age"])
    except ValueError:
        age = 0
    if not passenger_details["name"] or not 1 <= age <= 120 or passenger_details["gender"] not in {"Female", "Male", "Non-binary", "Prefer not to say"}:
        flash("Enter a valid passenger name, age, and gender.", "danger")
        return redirect(url_for("passenger_details", package_index=package_index))
    if len(passenger_details["phone"]) < 7 or "@" not in passenger_details["email"]:
        flash("Enter a valid phone number and email address.", "danger")
        return redirect(url_for("passenger_details", package_index=package_index))
    if passenger_details["payment_mode"] not in {"UPI · Google Pay / PhonePe / Paytm", "Credit card", "Debit card", "Net banking", "Wallets", "Pay later"}:
        flash("Choose a payment mode before confirming the booking.", "danger")
        return redirect(url_for("passenger_details", package_index=package_index))
    ...
    try:
        ...
        pdf_content = build_itinerary_pdf(itinerary)
        send_transaction_email(
            ...
            f"Lead passenger: {passenger_details['name']}\nPayment mode: {passenger_details['payment_mode']} (simulated)\n"
            f"Booking references: {refs}\nTotal itinerary cost: {rupee(itinerary.total_cost)}\n\n"
            "Your professional itinerary PDF is attached. You can also download it from Roamwise.",
            attachment=(f"roamwise-{itinerary.destination_city.lower()}-{itinerary.id}.pdf", pdf_content, "application", "pdf"),
        )
    except (OSError, smtplib.SMTPException):
        app.logger.warning("Unable to send booking confirmation email", exc_info=True)
    session.pop("draft_trip", None)
    session.pop("passenger_details", None)
...
@app.route("/book-package/<int:package_index>/passenger-details")
@login_required
def passenger_details(package_index):
    trip = session.get("draft_trip")
    if not trip:
        flash("Create an itinerary before booking it.", "warning")
        return redirect(url_for("planner"))
    ...
    if package_index < 0 or package_index >= len(packages):
        flash("That package is no longer available.", "danger")
        return redirect(url_for("planner"))
    passenger = session.get("passenger_details", {})
    return render_template("passenger_details.html", package=packages[package_index], package_index=package_index,
                           travelers=trip["travelers"], passenger=passenger, user=current_user)


@app.route("/book-package/<int:package_index>/payment", methods=["POST"])
@login_required
def payment_gateway(package_index):
    trip = session.get("draft_trip")
    if not trip:
        flash("Create an itinerary before booking it.", "warning")
        return redirect(url_for("planner"))
    ...
    if not passenger_details["name"] or not passenger_details["age"] or not passenger_details["gender"] or not passenger_details["phone"] or not passenger_details["email"] or not passenger_details["payment_mode"]:
        flash("Complete all required passenger and payment details first.", "danger")
        return redirect(url_for("passenger_details", package_index=package_index))
    departure = datetime.fromisoformat(trip["departure"]).date()
    returning = datetime.fromisoformat(trip["returning"]).date()
    packages = trip_options(trip["source"], trip["destination"], departure, returning, trip["travelers"], trip["budget"], trip["style"], trip["interests"], trip["hotel_preference"])
    if package_index < 0 or package_index >= len(packages):
        flash("That package is no longer available.", "danger")
        return redirect(url_for("planner"))
    session["passenger_details"] = passenger_details
    return render_template("payment_gateway.html", package=packages[package_index], package_index=package_index,
                           passenger=passenger_details, user=current_user)
...
def download_itinerary(itinerary_id):
    ...
    if itinerary.user_id != current_user.id:
        ...
        return redirect(url_for("dashboard"))
    return send_file(BytesIO(build_itinerary_pdf(itinerary)), as_attachment=True,
                     download_name=f"roamwise-{itinerary.destination_city.lower()}-{itinerary.id}.pdf", mimetype="application/pdf")
...
def build_itinerary_pdf(itinerary):
    ...
    pdfmetrics.registerFont(TTFont("RoamwiseSans", r"C:\Windows\Fonts\arial.ttf"))
    pdfmetrics.registerFont(TTFont("RoamwiseSansBold", r"C:\Windows\Fonts\arialbd.ttf"))
    styles["Normal"].fontName = "RoamwiseSans"
    styles["Title"].fontName = "RoamwiseSansBold"
    styles["Heading2"].fontName = "RoamwiseSansBold"
    styles["BodyText"].fontName = "RoamwiseSans"
    styles.add(ParagraphStyle(name="Brand", parent=styles["Title"], fontName="RoamwiseSansBold", fontSize=25, leading=28, textColor=colors.HexColor("#0b2637"), spaceAfter=2))
    styles.add(ParagraphStyle(name="Subtitle", parent=styles["Normal"], fontName="RoamwiseSans", fontSize=10, textColor=colors.HexColor("#60727a"), leading=13))
    styles.add(ParagraphStyle(name="SmallMuted", parent=styles["Normal"], fontName="RoamwiseSans", fontSize=8, textColor=colors.HexColor("#60727a"), leading=11))
    styles.add(ParagraphStyle(name="SectionTitle", parent=styles["Heading2"], fontName="RoamwiseSansBold", fontSize=13, textColor=colors.HexColor("#0b2637"), spaceBefore=14, spaceAfter=7))
    styles.add(ParagraphStyle(name="Amount", parent=styles["Normal"], fontName="RoamwiseSans", fontSize=9, alignment=TA_RIGHT, textColor=colors.HexColor("#0b2637")))
    styles.add(ParagraphStyle(name="Footer", parent=styles["Normal"], fontName="RoamwiseSans", fontSize=7, alignment=TA_CENTER, textColor=colors.HexColor("#60727a")))
    passenger = json.loads(itinerary.passenger_details or "{}")
    reference_rows = [[Paragraph("Booking type", styles["SmallMuted"]), Paragraph("Item", styles["SmallMuted"]), Paragraph("Reference", styles["SmallMuted"]), Paragraph("Amount", styles["SmallMuted"])] ]
    reference_rows.extend([[booking.booking_type, Paragraph(booking.item_name, styles["SmallMuted"]), booking.booking_reference or f"RW-{booking.id:06d}", Paragraph(rupee(booking.total_price), styles["Amount"])] for booking in itinerary.bookings])
    guidance = [
        "Carry a valid original photo ID matching the passenger name supplied during booking.",
        "Reach the departure point at least 30 minutes early and confirm the platform or pickup point before leaving.",
        "Keep each booking reference accessible. Separate transport, hotel, and activity items may have different policies.",
        "For activities, follow the local operator's timing, safety instructions, and weather guidance.",
    ]
    terms = [
        "This document is a Roamwise itinerary summary for the simulated booking environment.",
        "Prices, availability, timings, and cancellation outcomes are subject to the provider's final confirmation.",
        "Roamwise is not responsible for delays, closures, weather, or changes made by third-party operators.",
        "Contact the provider directly for changes or refunds and keep your booking references when requesting support.",
    ]
    story = [
        Paragraph("ROAMWISE", styles["Brand"]),
        Paragraph("TRAVEL ITINERARY  /  BOOKING CONFIRMATION", styles["Subtitle"]),
        Spacer(1, 6),
        Paragraph(f"<b>{itinerary.source_city} → {itinerary.destination_city}</b>  ·  {itinerary.departure_date:%d %b %Y} – {itinerary.return_date:%d %b %Y}", styles["Normal"]),
        Paragraph(f"Confirmation number: RW-IT-{itinerary.id:06d} | {itinerary.travelers} traveler(s) | Total: {rupee(itinerary.total_cost)}", styles["SmallMuted"]),
        Spacer(1, 14),
    ]
    story.append(Paragraph("Lead passenger", styles["SectionTitle"]))
    passenger_rows = [["Full name", passenger.get("name", "Not provided"), "Age", passenger.get("age", "-")],
                      ["Gender", passenger.get("gender", "-") , "Mobile", passenger.get("phone", "-")],
                      ["Email", passenger.get("email", "-") , "Payment", f"{passenger.get('payment_mode', 'Not selected')} (simulated)"],
                      ["Travel ID", f"{passenger.get('id_type', 'Not provided')} ending {passenger.get('id_last4', '----')}" if passenger.get("id_type") else "Not provided", "Status", "Pending provider confirmation"]]
    passenger_table = Table(passenger_rows, colWidths=[25 * mm, 62 * mm, 25 * mm, 55 * mm])
    passenger_table.setStyle(TableStyle([("FONTNAME", (0, 0), (-1, -1), "RoamwiseSans"), ("BACKGROUND", (0, 0), (0, -1), colors.HexColor("#e7f5f3")), ("BACKGROUND", (2, 0), (2, -1), colors.HexColor("#e7f5f3")), ("GRID", (0, 0), (-1, -1), .25, colors.HexColor("#c8d8d7")), ("VALIGN", (0, 0), (-1, -1), "TOP"), ("PADDING", (0, 0), (-1, -1), 6)]))
    story.extend([passenger_table, Paragraph("Booking references", styles["SectionTitle"])])
    reference_table = Table(reference_rows, colWidths=[25 * mm, 72 * mm, 42 * mm, 28 * mm], repeatRows=1)
    reference_table.setStyle(TableStyle([("FONTNAME", (0, 0), (-1, -1), "RoamwiseSans"), ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0b2637")), ("TEXTCOLOR", (0, 0), (-1, 0), colors.white), ("GRID", (0, 0), (-1, -1), .25, colors.HexColor("#c8d8d7")), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.white, colors.HexColor("#f2f8f7")]), ("VALIGN", (0, 0), (-1, -1), "TOP"), ("PADDING", (0, 0), (-1, -1), 6)]))
    story.extend([reference_table, Paragraph("Day-by-day itinerary", styles["SectionTitle"])])
    ...
    story.extend([table, Paragraph("How to travel", styles["SectionTitle"]), Paragraph("<br/>".join(f"• {item}" for item in guidance), styles["SmallMuted"]), Paragraph("Terms and conditions", styles["SectionTitle"]), Paragraph("<br/>".join(f"• {item}" for item in terms), styles["SmallMuted"]), Spacer(1, 14), Paragraph("Thank you for travelling with Roamwise · Keep this PDF with your booking references", styles["Footer"])])

    def draw_page(canvas, doc):
        canvas.saveState()
        canvas.setStrokeColor(colors.HexColor("#d6e5e3"))
        canvas.line(18 * mm, 13 * mm, A4[0] - 18 * mm, 13 * mm)
        canvas.setFont("RoamwiseSans", 7)
        canvas.setFillColor(colors.HexColor("#60727a"))
        canvas.drawString(18 * mm, 8 * mm, "Roamwise · Travel planner")
...
@app.route("/train-booking", methods=["GET", "POST"])
@login_required
def train_booking():
    form = {
        ...
        "travel_class": request.form.get("travel_class", "3A"),
    ...
    if request.method == "POST":
        try:
            passengers = max(1, min(6, int(form["passengers"])))
        except ValueError:
            passengers = 0
        if not form["source"] or not form["destination"]:
            search_error = "Enter both the boarding city and destination."
        elif form["source"].casefold() == form["destination"].casefold():
            search_error = "Boarding city and destination must be different."
        elif not form["travel_date"] or form["travel_date"] < date.today().isoformat():
            search_error = "Choose today or a future travel date."
        elif not passengers:
            search_error = "Passengers must be between 1 and 6."
        else:
            source_matches = [match for match in search_world_cities(form["source"], limit=5) if match.get("country", "").casefold() == "india"]
            destination_matches = [match for match in search_world_cities(form["destination"], limit=5) if match.get("country", "").casefold() == "india"]
            if not source_matches or not destination_matches:
                search_error = "We could not resolve both places in India. Try a city or railway station name."
                return render_template("train_booking.html", form=form, search_results=None, search_error=search_error,
                                       locations=None, today=date.today().isoformat())
            else:
                source_match = source_matches[0]
                destination_match = destination_matches[0]
                source = source_match["name"]
                destination = destination_match["name"]
                locations = {"source": source_match, "destination": destination_match}
            route_key = f"{source}:{destination}:{form['travel_date']}"
            direct_available = sum(ord(character) for character in route_key) % 3 != 0
            class_label = {"1A": "First AC", "2A": "2A", "3A": "3A", "SL": "Sleeper"}.get(form["travel_class"], "3A")
            direct = {
                "type": "direct", "label": "Direct train", "title": f"{source} → {destination}",
                "segments": [{"mode": "Train", "name": "Roamwise Express 129", "from": source, "to": destination,
                              "depart": "06:20", "arrive": "22:10", "duration": "15h 50m", "class": class_label,
                              "availability": "34 seats available" if direct_available else "Waitlist 18", "price": 1840},
                             ],
                "total": 1840 * passengers, "risk": "Lowest risk · one train, one PNR" if direct_available else "Unavailable end to end",
                "available": direct_available,
            }
            split = {
                "type": "split", "label": "Split train journey", "title": f"{source} → Junction → {destination}",
                "segments": [{"mode": "Train", "name": "Roamwise Express 129", "from": source, "to": "Nagpur Junction",
                              "depart": "06:20", "arrive": "16:45", "duration": "10h 25m", "class": class_label,
                              "availability": "34 seats available", "price": 980},
                             {"mode": "Train", "name": "Ganga Link 208", "from": "Nagpur Junction", "to": destination,
                              "depart": "19:30", "arrive": "07:15 next day", "duration": "11h 45m", "class": class_label,
                              "availability": "12 seats available", "price": 1120}],
                "total": 2100 * passengers, "risk": "Medium risk · 2 tickets, 2 PNRs · 2h 45m transfer buffer", "available": True,
            }
            multimodal = {
                "type": "multimodal", "label": "Train + bus fallback", "title": f"{source} → Railhead → {destination}",
                "segments": [{"mode": "Train", "name": "Roamwise Express 129", "from": source, "to": "Patna Railhead",
                              "depart": "06:20", "arrive": "18:10", "duration": "11h 50m", "class": class_label,
                              "availability": "18 seats available", "price": 1510},
                             {"mode": "Bus", "name": "InterCity Sleeper 77", "from": "Patna Railhead", "to": destination,
                              "depart": "20:00", "arrive": "23:30", "duration": "3h 30m", "class": "Sleeper", "availability": "Seats available", "price": 650}],
                "total": 2160 * passengers, "risk": "Higher risk · separate operators and tickets", "available": True,
            }
            search_results = [direct, split, multimodal] if direct_available else [split, multimodal]
            for result in search_results:
                result["total_display"] = rupee(result["total"])
                for segment in result["segments"]:
                    segment["price_display"] = rupee(segment["price"] * passengers)
            form["passengers"] = str(passengers)
    return render_template("train_booking.html", form=form, search_results=search_results, search_error=search_error,
                           locations=locations, today=date.today().isoformat())
...
def upgrade_existing_schema():
    ...
    if db.engine.dialect.name == "sqlite":
        ...
        db.session.commit()
    itinerary_columns = {column["name"] for column in inspect(db.engine).get_columns("itinerary")}
    if "passenger_details" not in itinerary_columns:
        db.session.execute(text("ALTER TABLE itinerary ADD COLUMN passenger_details TEXT"))
        db.session.commit()
